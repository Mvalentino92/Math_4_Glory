...
public class iQueens
{
	...
	public static int[] depth = new int[size];
	...
	//Iterative method that uses backtracking by storing relevant information
	public static void queens()
	{
		while(true)
		{
			...
			if(currentRow == size)
			{
				...
				count++;
				currentRow--;
				markCol[depth[currentRow]] = 0;
			       	board[currentRow][depth[currentRow]] = 0;
				currentCol = depth[currentRow] + 1;
			}
			...
			{
				//If this move is valid, update all variables and jump to next row.
				if(validMove(currentRow,i))
				{
					//Keep track of the depth and what column was chosen, update board 
			...
			{
				//Backtrack by accessing stored information about the depth and columns
				currentRow--;
